...
public class ClassSetterMap{
    private static <O,V> Setter<O,V> generateSetter(Class<O> clazz,Supplier<Class<V>> propertyTypeSupplier,Supplier<String> propertyNameSupplier,boolean method) throws NotFoundException, CannotCompileException, InstantiationException, IllegalAccessException{
        return (Setter<O,V>)GetSetUtil.generateSetter(clazz,propertyTypeSupplier,propertyNameSupplier,method);
    }
    @SuppressWarnings("unchecked")
    private static <O,V> Setter<O,V> generateSetter(Class<O> clazz,Field field) throws NotFoundException, CannotCompileException, InstantiationException, IllegalAccessException{
        return (Setter<O,V>)generateSetter(clazz,()->{return field.getType();},()->{return field.getName();},false);
    }
    @SuppressWarnings("unchecked")
    ...
    @SuppressWarnings("unchecked")
    private static <O,V> Setter<O,V> generateSetter(Class<O> clazz,Method setter) throws InstantiationException, IllegalAccessException, NotFoundException, CannotCompileException{
        return (Setter<O,V>)generateSetter(clazz,()->{return setter.getParameterTypes()[0];},()->{return setter.getName();},true);
    }
    @SuppressWarnings("unchecked")
    ...
    private static <O,V> Setter<O,V> generateSetter(Class<O> clazz,String property) throws SecurityException, InstantiationException, IllegalAccessException, NotFoundException, CannotCompileException{
        try{
        ...
        String setterName=ThreadLocalStringBuilder.builder().append("set").append(property.substring(0,1).toUpperCase()).append(property.substring(1)).toString();
        Method setter=null;
